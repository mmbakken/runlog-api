...
const useFreshTokens = async (user) => {
  ...
  // Required fields
  if (user.stravaRefreshToken == null || user.stravaTokenExpiresAt == null) {
    throw new Error(
      'useFreshTokens requires user instance to have non-empty values for stravaRefreshToken, stravaTokenExpiresAt'
    )
  }

  const expiresAt = DateTime.fromISO(
    user.stravaTokenExpiresAt.toISOString()
  ).toUTC()
  ...
  if (user.stravaTokenExpiresAt < soon) {
    ...
    ;(user.stravaAccessToken = newTokens.stravaAccessToken),
      ...
      (user.stravaTokenExpiresAt =
        parseInt(newTokens.stravaTokenExpiresAt) * 1000)
