...
const isSameWeekAs = (isoDateA, isoDateB, weekStartsOn) => {
  ...
  if (isoDateA == null) {
    throw new Error('isoDateA is required. Format as an ISO-8601 date string like "yyyy-mm-dd".')
  }

  if (isoDateB == null) {
    throw new Error('isoDateB is required. Format as an ISO-8601 date string like "yyyy-mm-dd".')
  }

  if (weekStartsOn == null || typeof weekStartsOn !== 'number' || weekStartsOn < 1 || weekStartsOn > 7) {
    throw new Error('weekStartsOn is required. Must be an integer between 1 and 7 (inclusively).')
  }
  ...
  if (dateA == null) {
    throw new Error('isoDateA must be formatted as an ISO-8601 date string like "yyyy-mm-dd".')
  }
