...
const recalculatePlanRunIds = async (plan) => {
  if (plan == null) {
    console.error(
      'Cannot update training plan.dates runIds fields: plan is required.'
    )
    return
  }
  ...
  if (plan.dates == null) {
    console.error(
      'Cannot update training plan.dates runIds fields: plan.dates is required.'
    )
    return
  }
  ...
  if (plan.userId == null) {
    console.error(
      'Cannot update training plan.dates runIds fields: plan.userId is required.'
    )
    return
  }
  ...
  if (userId == null || typeof userId !== 'string') {
    console.error(
      'Cannot update training plan.dates runIds fields: plan.userId could not be coerced into a string.'
    )
    return
  }
  ...
  if (plan.startDate == null) {
    console.error(
      'Cannot update training plan.dates runIds fields: plan.startDate is required.'
    )
    return
  }
  ...
  if (plan.endDate == null) {
    console.error(
      'Cannot update training plan.dates runIds fields: plan.endDate is required.'
    )
    return
  }
  ...
  for (let planDate of plan.dates) {
    const dateISO = DateTime.fromJSDate(planDate.dateISO, {
      zone: 'utc',
    }).toISODate()
