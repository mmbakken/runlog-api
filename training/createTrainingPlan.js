...
const createTrainingPlan = async (req, res) => {
  ...
  const startDT = DateTime.fromISO(req.body.startDate, { zone: 'utc' }).startOf(
    'day'
  )
  const endDate = req.body.endDate // ISO Date like yyyy-mm-dd
  const endDT = DateTime.fromISO(req.body.endDate, { zone: 'utc' }).startOf(
    'day'
  )
  ...
  if (startDate == null || !startDT.isValid) {
    return res
      ...
      .json({
        error:
          'Unable to create training plan: startDate field must be a valid ISO 8601 date string like yyyy-mm-dd',
      })
  }
  ...
  if (endDate == null || !endDT.isValid) {
    return res
      ...
      .json({
        error:
          'Unable to create training plan: endDate field must be a valid ISO 8601 date string like yyyy-mm-dd',
      })
  }
  ...
  if (endDT < startDT) {
    return res
      ...
      .json({
        error:
          'Unable to create training plan: startDate must come before endDate',
      })
  }
  ...
  if (weekCount == null || !Number.isInteger(weekCount) || weekCount < 1) {
    return res
      ...
      .json({
        error:
          'Unable to create training plan: weekCount must be a positive integer',
      })
  }
  ...
  if (startDT.plus({ days: weekCount * 7 - 1 }).toISODate() !== endDate) {
    return res
      ...
      .json({
        error:
          'Unable to create training plan: weekCount must be exactly the difference between start and end dates',
      })
  }
  ...
  if (
    ...
    return res
      .status(400)
      .json({
        error: 'Unable to create training plan: timezone must be a string',
      })
  }
  ...
  if (goal == null || typeof goal !== 'string') {
    return res
      .status(400)
      .json({ error: 'Unable to create training plan: goal must be a string' })
  }
