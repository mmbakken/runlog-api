...
const updatePlanDateRunIds = (plan, dateISO, runId, addRun) => {
  ...
  for (let planDate of plan.dates) {
    const planDateISO = DateTime.fromJSDate(planDate.dateISO, {
      zone: 'utc',
    }).toISODate()
...
const updatePlansFromRun = async (run, userId, addRun) => {
  if (run == null) {
    console.error(
      'Cannot update training plan actualDistance fields: run is required.'
    )
    return
  }
  ...
  if (run.distance == null) {
    console.error(
      'Cannot update training plan actualDistance fields: run.distance is required.'
    )
    return
  }
  ...
  if (userId == null) {
    console.error(
      'Cannot update training plan actualDistance fields: userId is required.'
    )
    return
  }
  ...
  const startOfRunLocalDT = DateTime.fromJSDate(run.startDateLocal, {
    zone: 'utc',
  }).startOf('day')
  ...
  if (plans == null || plans.length === 0) {
    console.log(
      `updatePlansFromRun: No training plans found for this user that include local run start date of "${startOfRunLocalISOString}". Will not modify any plans.`
    )
    return
  }
  ...
  for (let plan of plans) {
    ...
    plan = await updatePlanDateRunIds(
      plan,
      startOfRunLocalISOString,
      run._id.toString(),
      addRun
    )
    await plan.save()
